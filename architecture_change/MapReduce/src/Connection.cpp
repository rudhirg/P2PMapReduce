...
int Connection::Connect( std::string ip, int port )
{
    ...
    if (ConnectSocket == INVALID_SOCKET) {
        Log(ERROR, L"Error at socket(): %ld\n", WSAGetLastError() );
        return -1;
    }
    ...
    if ( result == SOCKET_ERROR) {
        ...
        Log(ERROR, L"Unable to connect to remote peer: %ld\n", WSAGetLastError());
        return -1;
    }
...
int Connection::SendMessage(std::string mesg)
{
    ...
    if (result == SOCKET_ERROR) {
        Log(ERROR, L"Unable to send message to remote peer: %ld\n", WSAGetLastError());
        return -1;
    }
	return result;
}
...
void Connection::MsgListener()
{
	while(true) {
		...
		if( iResult > 0 ) {
			Log(CONSOLE, L"Received %d bytes on socket: %d - %s\n", iResult, m_connSocket, m_recvbuf);
		} else if( iResult == 0 ) {
			Log(CONSOLE, L"Connection on socket: %d closed\n", m_connSocket);
			break;
		} else {
			Log(ERROR, L"Connection on socket: %d closed - unexpectedly, error - %d\n", m_connSocket, WSAGetLastError());
			break;
		}
...
void Connection::ProcessMessages(std::string mesg)
{
	...
	if( pMsg == NULL ) {
		Log(ERROR, L"Message : %s could not be parsed\n", mesg.c_str());
		return;
	}
	...
	if( m_pPeerNode == NULL ) {
		Log(ERROR, L"Connection:: ProcessMessages - PeerNode found NULL\n");
		return;
	}
